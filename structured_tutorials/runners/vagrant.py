...
class VagrantRunner(RunnerBase):
    ...
    def get_machines(self, options: dict[str, Any]) -> list[str]:
        machines: str | list[str] = options.get("machine", "default")
        if not machines:
            raise RuntimeError("empty list of machines passed.")
    ...
    def prepare_tutorial(self) -> None:
        ...
        proc = self.vagrant(["up"])
        if proc.returncode != 0:
            raise RunTutorialException("Failed to run 'vagrant up'.")
